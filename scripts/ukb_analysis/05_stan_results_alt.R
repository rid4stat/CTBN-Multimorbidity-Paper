...
make_fit_fn <- function(prior, max_order, theta) {
  ...
  # Prior-specific hyperparameters (fixed across the grid)
  .pi0        <- 0.5
  .spike_var  <- 0.01
  .a0         <- 2.0
  .b0         <- 1.0
  ...
  function(dt, ...) {
    ctbn_fit(
      ...
      iter              = 1000,
      warmup            = 500,
      ...   # receives target_conditions from ctbn_cv_compare
    )
  }
}
...
start_time <- Sys.time()
with_progress(cv_thetas <- ctbn_cv_compare(
  ...
  k_folds = 2, eval_times = c(1, 2, 5, 7, 10),
  .progress = TRUE
))
end_time <- Sys.time()
...
save_tex(xtable(tbl_521,
                caption = "Mean tdAUC and Brier score at 5-year horizon by prior
...
conds       <- names(fit_network$pip_matrix)
PIP_THRESH  <- 0.50
...
compute_baseline_q1000 <- function(fit, DT) {
  ...
  results  <- list()
  for (target in names(fit$stan_fits)) {
    sfit <- fit$stan_fits[[target]]
    if (is.null(sfit)) next
    post     <- as.data.frame(sfit)
    g_draws  <- as.matrix(post[, grep("^gamma\\[", names(post)), drop = FALSE])
    cov_names <- all_covs
    if (ncol(g_draws) == length(cov_names)) {
      ...
      q_samples <- exp(as.numeric(g_draws %*% z_vec)) * 1000
    } else {
      q_samples <- rep(NA_real_, nrow(g_draws))
    }
    results[[target]] <- data.table(
      target      = target,
      q_mean_1000 = mean(q_samples, na.rm = TRUE),
      q_lo_1000   = quantile(q_samples, 0.025, na.rm = TRUE),
      q_hi_1000   = quantile(q_samples, 0.975, na.rm = TRUE)
    )
  }
  rbindlist(results)
}

q_baseline <- compute_baseline_q1000(fit_network, DT_wide3[, -1])
...
conditional_risk_table <- function(fit, DT_wide, tau = 5) {
  ...
  for (target in all_conds) {
    ...
    if (length(beta_cols) < length(influencers)) next
    bm <- colMeans(post[, beta_cols, drop = FALSE])[seq_along(influencers)]
...
if (nrow(int_tbl) > 0) {
  ...
  fig_526c <- ggplot() +
    # Synergistic excess ribbon: between joint and additive counterfactual
    geom_ribbon(data = curve_wide,
                aes(x = t,
                    ymin = pmin(F_additive, get(names(curve_wide)[grepl("^Both", names(curve_wide))[1]])),
                    ymax = pmax(F_additive, get(names(curve_wide)[grepl("^Both", names(curve_wide))[1]])),
                    group = triplet),
                fill = "#f4a582", alpha = 0.45) +
    # All four profile curves
    geom_line(data = curve_long,
              aes(x = t, y = F, colour = profile, linetype = profile_type,
                  group = profile), linewidth = 0.85) +
    # Additive counterfactual (dashed grey)
    geom_line(data = curve_wide,
              aes(x = t, y = F_additive, group = triplet),
              colour = "grey50", linetype = "dotted", linewidth = 0.7) +
    facet_wrap(~triplet, scales = "free_y", ncol = 2) +
    scale_colour_brewer(palette = "Set1", name = "Profile") +
    scale_linetype_manual(
      values  = c(baseline = "solid", main_effect = "solid",
                  joint = "solid", other = "dashed"),
      guide   = "none") +
    scale_y_continuous(labels = percent_format(accuracy = 1)) +
    scale_x_continuous(breaks = c(0, 2, 5, 10)) +
    labs(
...
n_cov       <- length(param_names) - 1

pip_cov_mat  <- matrix(0, n_cov, length(targets), dimnames = list(param_names[-1], targets))
gamma_mat    <- matrix(0, n_cov, length(targets), dimnames = list(param_names[-1], targets))
expgamma_mat <- matrix(0, n_cov, length(targets), dimnames = list(param_names[-1], targets))

for (t in targets) {
  pip_cov_mat[, t] <- fit_network$pip_cov_list[[t]][-1]
  df <- as.data.frame(fit_network$models[[t]])
  dg <- df[, grep("^gamma", names(df))]
  gamma_mat[, t]    <- colMeans(dg[, -1])
  expgamma_mat[, t] <- colMeans(exp(dg[, -1]))
}

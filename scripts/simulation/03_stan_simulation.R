# =============================================================================
# ctbn_simulation_v3.R  (progress monitoring edition)
# CTBN Simulation Study — Data Generation, Model Fitting, and Metrics
#
# KEY CHANGES FROM v2
# -------------------
# 1. PROGRESS MONITORING SYSTEM
#    - SimProgress R6 class: thread-safe, file-backed progress tracker
#      that works correctly across parallel workers.
#    - Two-level progress: outer (replicates) and inner (folds + conditions).
#    - Progress state persisted to sim_results/progress/ as JSON so it
#      survives crashes and can be resumed.
#    - Real-time console dashboard: ETA, elapsed time, completion %, bar.
#    - Per-scenario log files: sim_results/logs/scenario_X.log
#
# 2. RESUMABILITY
#    - Each replicate result cached individually to
#      sim_results/replicates/rep_<scenario>_<r>.rds
#    - run_simulation() skips already-completed replicates on restart.
#
# 3. ERROR TRACKING
#    - Failed replicates logged with error message and timestamp.
#    - Summary error report written on completion.
#
# 4. TIMING
#    - Per-replicate wall time tracked and reported.
#    - ETA computed as rolling mean of last 10 replicate times.
# =============================================================================

# --------------------------------------------------------------------------- #
# 0.  Packages
# --------------------------------------------------------------------------- #
suppressPackageStartupMessages({
  library(data.table)
  library(rstan)
  library(foreach)
  library(doParallel)
  library(pROC)
  library(timeROC)
  library(R6)
  library(jsonlite)
  library(crayon)       # coloured console output
})

source("ctbn_results_v4.R")   # ctbn_fit(), compute_F_m(), get_lp() etc.

# --------------------------------------------------------------------------- #
# 1.  Global settings
# --------------------------------------------------------------------------- #
SIM_SEED    <- 2026L
N_SIM       <- 5000L
N_REP       <- 100L
K_FOLD      <- 5L
EVAL_TIMES  <- c(1, 2, 5, 7, 10)
T_HORIZON   <- 10
P_MAX_TRUE  <- 3L
P_FIT_GRID  <- c(1L, 2L, 3L)
THETA_GRID  <- c(0.5, 1.0, 2.0)
THETA_FIT   <- 1.0
PRIOR_FIT   <- c("spike_slab", "structured", "lasso", "horseshoe")
PIP_THRESH  <- 0.50

# Directories
for (d in c("sim_results","sim_results/replicates","sim_results/figures",
            "sim_results/tables","sim_results/logs","sim_results/progress"))
  dir.create(d, showWarnings = FALSE, recursive = TRUE)

# --------------------------------------------------------------------------- #
# 2.  Progress monitoring system
# --------------------------------------------------------------------------- #

# ── Console progress bar helper ───────────────────────────────────────────── #
# Works from any process (main or worker) by writing to a shared JSON file.

make_progress_bar <- function(current, total, width = 40) {
  frac   <- min(current / max(total, 1), 1)
  filled <- round(frac * width)
  bar    <- paste0(
    strrep("\u2588", filled),
    strrep("\u2591", width - filled))
  sprintf("[%s] %3.0f%%  %d/%d", bar, frac * 100, current, total)
}

format_duration <- function(secs) {
  secs <- as.numeric(secs)
  if (secs < 60)  return(sprintf("%.0fs", secs))
  if (secs < 3600) return(sprintf("%.0fm %.0fs", secs %/% 60, secs %% 60))
  sprintf("%.0fh %.0fm", secs %/% 3600, (secs %% 3600) %/% 60)
}

# ── SimProgress: file-backed thread-safe progress state ──────────────────── #
SimProgress <- R6::R6Class("SimProgress",
  private = list(
    state_file  = NULL,
    lock_file   = NULL,
    scenario    = NULL,
    n_rep       = NULL,
    k_fold      = NULL,
    n_cond      = NULL,
    start_time  = NULL,
    rep_times   = NULL,   # rolling vector of per-replicate wall times

    acquire_lock = function(timeout = 30) {
      t0 <- proc.time()["elapsed"]
      while (file.exists(private$lock_file)) {
        if (proc.time()["elapsed"] - t0 > timeout) {
          warning("Progress lock timeout — proceeding anyway")
          break
        }
        Sys.sleep(0.05)
      }
      writeLines(as.character(Sys.getpid()), private$lock_file)
    },

    release_lock = function() {
      if (file.exists(private$lock_file))
        try(file.remove(private$lock_file), silent = TRUE)
    },

    read_state = function() {
      if (!file.exists(private$state_file))
        return(list(completed = 0L, failed = 0L, rep_times = numeric(0),
                    errors = list()))
      tryCatch(jsonlite::fromJSON(private$state_file),
               error = function(e) list(completed = 0L, failed = 0L,
                                        rep_times = numeric(0),
                                        errors = list()))
    },

    write_state = function(state) {
      tryCatch(jsonlite::write_json(state, private$state_file, auto_unbox = TRUE),
               error = function(e) invisible(NULL))
    }
  ),

  public = list(
    initialize = function(scenario, n_rep, k_fold, n_cond) {
      private$scenario   <- scenario
      private$n_rep      <- n_rep
      private$k_fold     <- k_fold
      private$n_cond     <- n_cond
      private$start_time <- proc.time()["elapsed"]
      private$state_file <- file.path(
        "sim_results/progress",
        sprintf("state_%s.json", scenario))
      private$lock_file  <- file.path(
        "sim_results/progress",
        sprintf(".lock_%s", scenario))
      invisible(self)
    },

    # Called by each worker when a replicate finishes successfully
    increment = function(rep_idx, wall_time) {
      private$acquire_lock()
      on.exit(private$release_lock())
      state <- private$read_state()
      state$completed  <- state$completed + 1L
      state$rep_times  <- c(tail(state$rep_times, 9), wall_time)
      private$write_state(state)
      invisible(self)
    },

    # Called by each worker when a replicate fails
    record_failure = function(rep_idx, msg) {
      private$acquire_lock()
      on.exit(private$release_lock())
      state <- private$read_state()
      state$failed <- state$failed + 1L
      state$errors <- c(state$errors,
                        list(list(rep   = rep_idx,
                                  msg   = as.character(msg),
                                  time  = format(Sys.time()))))
      private$write_state(state)
      invisible(self)
    },

    # Print dashboard — call from main process polling loop
    print_dashboard = function() {
      state    <- private$read_state()
      done     <- state$completed
      failed   <- state$failed
      n        <- private$n_rep
      elapsed  <- proc.time()["elapsed"] - private$start_time
      bar      <- make_progress_bar(done, n)

      eta_str <- ""
      if (done > 0 && length(state$rep_times) > 0) {
        mean_t <- mean(state$rep_times)
        eta    <- mean_t * (n - done)
        eta_str <- sprintf("  ETA: %s", format_duration(eta))
      }

      cat("\r\033[K")   # clear line
      cat(sprintf(
        "%s [Scen %s]  Elapsed: %s%s  Failed: %d",
        bar, private$scenario,
        format_duration(elapsed), eta_str, failed))
      utils::flush.console()
      invisible(state)
    },

    # Full status report — call at end
    final_report = function() {
      state   <- private$read_state()
      elapsed <- proc.time()["elapsed"] - private$start_time
      cat(sprintf(
        "\n\n%s Scenario %s complete\n",
        format(Sys.time(), "%Y-%m-%d %H:%M:%S"),
        private$scenario))
      cat(sprintf("  Replicates completed : %d / %d\n",
                  state$completed, private$n_rep))
      cat(sprintf("  Replicates failed    : %d\n", state$failed))
      cat(sprintf("  Total elapsed        : %s\n", format_duration(elapsed)))
      if (length(state$rep_times) > 0)
        cat(sprintf("  Mean rep time        : %s\n",
                    format_duration(mean(state$rep_times))))
      if (state$failed > 0) {
        cat("  Errors:\n")
        for (e in state$errors)
          cat(sprintf("    Rep %03d [%s]: %s\n", e$rep, e$time,
                      substr(e$msg, 1, 120)))
      }
      invisible(state)
    },

    # Check if replicate already done
    rep_done = function(rep_idx, scenario) {
      path <- file.path("sim_results/replicates",
                        sprintf("rep_%s_%03d.rds", scenario, rep_idx))
      file.exists(path)
    },

    n_remaining = function(scenario) {
      paths <- file.path("sim_results/replicates",
        sprintf("rep_%s_%03d.rds", scenario, seq_len(private$n_rep)))
      sum(!file.exists(paths))
    }
  )
)


# ── Per-replicate console logger (worker side) ────────────────────────────── #
rep_log <- function(scenario, rep_idx, msg, level = "INFO") {
  log_path <- file.path("sim_results/logs",
                         sprintf("scenario_%s.log", scenario))
  line <- sprintf("[%s] [%s] [Rep %03d] %s\n",
                  format(Sys.time(), "%H:%M:%S"), level, rep_idx, msg)
  tryCatch(cat(line, file = log_path, append = TRUE),
           error = function(e) invisible(NULL))
}


# ── Inner progress: fold + condition level (printed to log only) ──────────── #
inner_progress <- function(scenario, rep_idx, phase, item, total_items) {
  rep_log(scenario, rep_idx,
          sprintf("%s | %s (%d/%d)", phase, item, total_items[[item]], total_items[["total"]]),
          level = "DETAIL")
}

# --------------------------------------------------------------------------- #
# 3.  True network parameters (same as v2)
# --------------------------------------------------------------------------- #

CONDS <- c("HTN","IHD","MN","OA","DM","HL","DVL","RS","DE","CLRD")
M     <- length(CONDS)

TRUE_BETA0 <- setNames(log(c(
  HTN=0.150,IHD=0.080,MN=0.005,OA=0.060,DM=0.050,
  HL=0.040,DVL=0.040,RS=0.080,DE=0.070,CLRD=0.050)), CONDS)

TRUE_GRAPH <- list(
  HTN=c("DM","IHD","RS"), IHD=c("HTN","DM"), MN=c("HTN"),
  OA=c("DM","HTN"),       DM=c("HTN","IHD"),
  HL=c("OA","HTN"),       DVL=c("OA","DM"),
  RS=c("DE","CLRD"),      DE=c("RS","OA"),
  CLRD=c("RS","DM","HTN"))

TRUE_BETA_MAIN <- c(
  "HTN|DM"=0.60,"HTN|IHD"=0.25,"HTN|RS"=0.20,
  "IHD|HTN"=0.35,"IHD|DM"=0.55,
  "MN|HTN"=0.18,
  "OA|DM"=0.28,"OA|HTN"=0.22,
  "DM|HTN"=0.30,"DM|IHD"=0.35,
  "HL|OA"=0.32,"HL|HTN"=0.28,
  "DVL|OA"=0.25,"DVL|DM"=0.30,
  "RS|DE"=0.40,"RS|CLRD"=0.45,
  "DE|RS"=0.42,"DE|OA"=0.20,
  "CLRD|RS"=0.50,"CLRD|DM"=0.35,"CLRD|HTN"=0.28)

make_true_interactions_2way <- function(seed = SIM_SEED) {
  set.seed(seed)
  result <- list()
  for (m in names(TRUE_GRAPH)) {
    pa <- TRUE_GRAPH[[m]]
    if (length(pa) < 2) next
    for (pair in combn(pa, 2, simplify = FALSE)) {
      key <- paste(m, pair[1], pair[2], sep = "|")
      result[[key]] <- if (runif(1) < 0.20)
        (if (runif(1) < 0.70) rnorm(1,0,0.3) else rnorm(1,0.8,0.2)) else 0
    }
  }
  result
}

make_true_interactions_3way <- function(seed = SIM_SEED + 99L) {
  set.seed(seed)
  result <- list()
  for (m in names(TRUE_GRAPH)) {
    pa <- TRUE_GRAPH[[m]]
    if (length(pa) < 3) next
    for (triple in combn(pa, 3, simplify = FALSE)) {
      key <- paste(c(m, triple), collapse = "|")
      result[[key]] <- if (runif(1) < 0.10)
        (if (runif(1) < 0.70) rnorm(1,0,0.2) else rnorm(1,0.4,0.15)) else 0
    }
  }
  result
}

TRUE_BETA_INT2 <- make_true_interactions_2way()
TRUE_BETA_INT3 <- make_true_interactions_3way()

TRUE_GAMMA <- list(
  HTN =c(age=0.35,sex_male=0.20,smk_current=0.25,smk_former=0.10),
  IHD =c(age=0.30,sex_male=0.25,smk_current=0.40,smk_former=0.15),
  MN  =c(age=0.25,sex_male=0.15,smk_current=0.35,smk_former=0.10),
  OA  =c(age=0.45,sex_male=0.10,smk_current=0.15,smk_former=0.05),
  DM  =c(age=0.25,sex_male=0.15,smk_current=0.30,smk_former=0.12),
  HL  =c(age=0.40,sex_male=0.30,smk_current=0.20,smk_former=0.10),
  DVL =c(age=0.20,sex_male=0.10,smk_current=0.20,smk_former=0.08),
  RS  =c(age=0.10,sex_male=0.05,smk_current=0.15,smk_former=0.05),
  DE  =c(age=0.05,sex_male=-0.10,smk_current=0.10,smk_former=0.05),
  CLRD=c(age=0.30,sex_male=0.20,smk_current=0.60,smk_former=0.20))

# --------------------------------------------------------------------------- #
# 4.  Data generation (same as v2)
# --------------------------------------------------------------------------- #

compute_q_true <- function(m, X_state, covs, include_3way = TRUE) {
  lp <- TRUE_BETA0[m]
  pa <- TRUE_GRAPH[[m]]
  for (j in pa) {
    key <- paste(m, j, sep = "|")
    if (key %in% names(TRUE_BETA_MAIN))
      lp <- lp + TRUE_BETA_MAIN[key] * X_state[j]
  }
  if (length(pa) >= 2)
    for (pair in combn(pa, 2, simplify = FALSE)) {
      key <- paste(m, pair[1], pair[2], sep = "|")
      if (key %in% names(TRUE_BETA_INT2) && TRUE_BETA_INT2[[key]] != 0)
        lp <- lp + TRUE_BETA_INT2[[key]] * X_state[pair[1]] * X_state[pair[2]]
    }
  if (include_3way && length(pa) >= 3)
    for (triple in combn(pa, 3, simplify = FALSE)) {
      key <- paste(c(m, triple), collapse = "|")
      if (key %in% names(TRUE_BETA_INT3) && TRUE_BETA_INT3[[key]] != 0)
        lp <- lp + TRUE_BETA_INT3[[key]] *
          X_state[triple[1]] * X_state[triple[2]] * X_state[triple[3]]
    }
  gamma_m <- TRUE_GAMMA[[m]]
  lp <- lp + sum(gamma_m * covs[names(gamma_m)])
  exp(lp)
}

sim_smoking_step <- function(smk, dt, q01=0.04, q12=0.06) {
  if (smk == 2L) return(2L)
  if (smk == 0L) return(if (runif(1) < 1-exp(-q01*dt)) 1L else 0L)
  if (runif(1) < 1-exp(-q12*dt)) 2L else 1L
}

smk_to_covs <- function(smk)
  c(smk_current = as.numeric(smk == 1L), smk_former = as.numeric(smk == 2L))

simulate_patient <- function(eid, t_horizon=T_HORIZON, dt_step=0.25,
                              include_3way=TRUE) {
  init_prev <- c(HTN=0.16,IHD=0.08,MN=0.06,OA=0.06,DM=0.05,
                 HL=0.04,DVL=0.04,RS=0.08,DE=0.07,CLRD=0.05)
  X_state   <- setNames(rbinom(M,1,init_prev), CONDS)
  age0      <- rnorm(1,57,9.39)
  sex_male  <- rbinom(1,1,0.56)
  smk       <- sample(0:2,1,prob=c(0.47,0.11,0.42))
  records   <- list()
  t_now     <- 0

  while (t_now < t_horizon) {
    t_next   <- min(t_now + dt_step, t_horizon)
    dt       <- t_next - t_now
    smk_covs <- smk_to_covs(smk)
    covs     <- c(age=age0+t_now, sex_male=sex_male, smk_covs)
    at_risk  <- CONDS[X_state == 0]

    if (length(at_risk) == 0) {
      smk <- sim_smoking_step(smk, dt); t_now <- t_next; next
    }

    q_vec  <- setNames(
      sapply(at_risk, compute_q_true, X_state=X_state, covs=covs,
             include_3way=include_3way), at_risk)
    Lambda <- sum(q_vec)
    t_cand <- rexp(1, max(Lambda, 1e-10))

    if (t_cand < dt) {
      m_ev    <- sample(at_risk, 1, prob=q_vec/Lambda)
      t_ev    <- t_now + t_cand
      records[[length(records)+1]] <- data.table(
        eid=eid, time_to_event=t_now, dt=t_ev-t_now,
        matrix(t(X_state),nrow=1,dimnames=list(NULL,CONDS)),
        age=age0+t_now, sex_male=sex_male,
        smk_current=smk_covs["smk_current"],
        smk_former =smk_covs["smk_former"],
        event_cond=m_ev)
      X_state[m_ev] <- 1L
      t_now <- t_ev
      if (all(X_state==1)) break
    } else {
      records[[length(records)+1]] <- data.table(
        eid=eid, time_to_event=t_now, dt=dt,
        matrix(t(X_state),nrow=1,dimnames=list(NULL,CONDS)),
        age=age0+t_now, sex_male=sex_male,
        smk_current=smk_covs["smk_current"],
        smk_former =smk_covs["smk_former"],
        event_cond=NA_character_)
      smk   <- sim_smoking_step(smk, dt)
      t_now <- t_next
    }
  }
  if (length(records)==0) return(NULL)
  rbindlist(records)
}

generate_dataset <- function(n=N_SIM, seed=NULL, include_3way=TRUE) {
  if (!is.null(seed)) set.seed(seed)
  rbindlist(lapply(seq_len(n), simulate_patient, include_3way=include_3way))
}

prepare_wide <- function(DT) {
  DT[, event_cond := NULL]
  DT[, (CONDS) := lapply(.SD, as.integer), .SDcols=CONDS]
  DT
}

# --------------------------------------------------------------------------- #
# 5.  Metrics (same as v2, with rep_log calls added)
# --------------------------------------------------------------------------- #

compute_oracle_metrics <- function(DT_test, include_3way=TRUE) {
  results <- list()
  for (m in CONDS) {
    ec  <- paste0(m,"_event")
    atr <- DT_test[get(m)==0 & dt>0]
    if (nrow(atr)==0 || !ec %in% names(atr)) next
    y <- atr[[ec]]; dt_ <- atr$dt
    q_true <- vapply(seq_len(nrow(atr)), function(ri) {
      row  <- atr[ri]
      X_st <- unlist(row[,..CONDS])
      covs <- c(age=row$age, sex_male=row$sex_male,
                smk_current=row$smk_current, smk_former=row$smk_former)
      compute_q_true(m, X_st, covs, include_3way)
    }, numeric(1))
    mu  <- q_true * pmax(dt_,1e-10)
    p   <- 1 - exp(-mu)
    results[[m]] <- data.table(
      condition    = m,
      oracle_pll   = mean(dpois(y,pmax(mu,1e-15),log=TRUE), na.rm=TRUE),
      oracle_brier = mean((y-p)^2, na.rm=TRUE))
  }
  rbindlist(results)
}

permn_3 <- function(x)
  matrix(c(x[1],x[2],x[3], x[1],x[3],x[2],
           x[2],x[1],x[3], x[2],x[3],x[1],
           x[3],x[1],x[2], x[3],x[2],x[1]),
         ncol=3, byrow=TRUE)

compute_selection_metrics <- function(fit, pip_thresh=PIP_THRESH, p_true=2L) {
  true_main_nz <- names(TRUE_BETA_MAIN)[TRUE_BETA_MAIN != 0]
  true_2way_nz <- names(TRUE_BETA_INT2)[unlist(TRUE_BETA_INT2) != 0]
  true_3way_nz <- if (p_true>=3L)
    names(TRUE_BETA_INT3)[unlist(TRUE_BETA_INT3)!=0] else character(0)

  pip_mat   <- fit$pip_matrix
  kappa_mat <- fit$kappa_matrix
  prior     <- fit$prior
  p_fit     <- fit$call_args$max_order

  mk_score <- function(post, pip_cols, kappa_mat, j, m, col_pos) {
    if (prior == "spike_slab" && length(pip_cols) >= col_pos)
      return(mean(post[[pip_cols[col_pos]]]))
    if (!is.null(kappa_mat)) {
      kv <- kappa_mat[j, m]
      if (!is.na(kv)) return(1 - kv)
    }
    NA_real_
  }

  r_main <- r_2way <- r_3way <- list()

  for (m in CONDS) {
    sfit <- fit$stan_fits[[m]]; if (is.null(sfit)) next
    infl <- setdiff(CONDS, m)
    post <- as.data.frame(sfit)
    bc   <- grep("^beta\\[",     names(post), value=TRUE)
    pc   <- grep("^pip_beta\\[", names(post), value=TRUE)
    tmp  <- as.data.frame(matrix(0,1,length(infl))); colnames(tmp) <- infl
    ir   <- build_interaction_cols(tmp, infl, p_fit)
    n_m  <- length(infl)

    for (idx in seq_along(infl)) {
      j   <- infl[idx]; key <- paste(m,j,sep="|")
      sc  <- mk_score(post, pc, kappa_mat, j, m, idx)
      r_main[[length(r_main)+1]] <- data.table(
        target=m, influencer=j,
        true_nz  = key %in% true_main_nz,
        selected = !is.na(sc) && sc >= pip_thresh,
        score    = sc)
    }

    if (p_fit >= 2L) {
      pairs <- combn(infl, 2, simplify=FALSE)
      for (pi in seq_along(pairs)) {
        j <- pairs[[pi]][1]; k <- pairs[[pi]][2]
        cn <- paste(j,k,sep="_x_"); cr <- paste(k,j,sep="_x_")
        ac <- if (cn %in% ir$cols) cn else if (cr %in% ir$cols) cr else NA
        if (is.na(ac)) next
        pos  <- which(ir$cols==ac) + n_m
        sc   <- mk_score(post, pc, kappa_mat, j, m, pos)
        k1   <- paste(m,j,k,sep="|"); k2 <- paste(m,k,j,sep="|")
        r_2way[[length(r_2way)+1]] <- data.table(
          target=m, cond_j=j, cond_k=k,
          true_nz  = k1 %in% true_2way_nz || k2 %in% true_2way_nz,
          selected = !is.na(sc) && sc >= pip_thresh,
          score    = sc)
      }
    }

    if (p_fit >= 3L && p_true >= 3L) {
      triples <- combn(infl, 3, simplify=FALSE)
      for (ti in seq_along(triples)) {
        j <- triples[[ti]][1]; k <- triples[[ti]][2]; l <- triples[[ti]][3]
        perms <- apply(permn_3(c(j,k,l)),1,function(x) paste(x,collapse="_x_"))
        ac    <- perms[perms %in% ir$cols]
        if (length(ac)==0) next
        pos   <- which(ir$cols==ac[1]) + n_m
        sc    <- mk_score(post, pc, kappa_mat, j, m, pos)
        keys  <- apply(permn_3(c(j,k,l)),1,function(x) paste(c(m,x),collapse="|"))
        r_3way[[length(r_3way)+1]] <- data.table(
          target=m, cond_j=j, cond_k=k, cond_l=l,
          true_nz  = any(keys %in% true_3way_nz),
          selected = !is.na(sc) && sc >= pip_thresh,
          score    = sc)
      }
    }
  }

  calc_sel <- function(dt) {
    if (is.null(dt) || nrow(dt)==0)
      return(list(tpr=NA_real_,fpr=NA_real_,auc=NA_real_))
    nz  <- dt[true_nz==TRUE]; z <- dt[true_nz==FALSE]
    tpr <- if (nrow(nz)>0) mean(nz$selected,na.rm=TRUE) else NA_real_
    fpr <- if (nrow(z)>0)  mean(z$selected, na.rm=TRUE) else NA_real_
    av  <- tryCatch(
      if (!all(is.na(dt$score)) && length(unique(dt$true_nz))==2)
        as.numeric(auc(roc(as.numeric(dt$true_nz),dt$score,
                           quiet=TRUE,direction="<"))) else NA_real_,
      error=function(e) NA_real_)
    list(tpr=tpr, fpr=fpr, auc=av)
  }

  ms <- calc_sel(rbindlist(r_main, fill=TRUE))
  ts <- calc_sel(rbindlist(r_2way, fill=TRUE))
  hs <- calc_sel(rbindlist(r_3way, fill=TRUE))
  data.table(level=c("main_effect","interaction_2way","interaction_3way"),
             tpr=c(ms$tpr,ts$tpr,hs$tpr),
             fpr=c(ms$fpr,ts$fpr,hs$fpr),
             selection_auc=c(ms$auc,ts$auc,hs$auc))
}

compute_recovery_metrics <- function(fit, p_true=2L) {
  p_fit   <- fit$call_args$max_order
  results <- list()

  for (m in CONDS) {
    sfit <- fit$stan_fits[[m]]; if (is.null(sfit)) next
    post <- as.data.frame(sfit)
    bc   <- grep("^beta\\[",  names(post), value=TRUE)
    gc   <- grep("^gamma\\[", names(post), value=TRUE)
    bm   <- colMeans(as.matrix(post[,bc, drop=FALSE]))
    bv   <- apply(  as.matrix(post[,bc, drop=FALSE]),2,var)
    bci  <- apply(  as.matrix(post[,bc, drop=FALSE]),2,quantile,c(.025,.975))
    gm   <- colMeans(as.matrix(post[,gc, drop=FALSE]))
    gv   <- apply(  as.matrix(post[,gc, drop=FALSE]),2,var)
    gci  <- apply(  as.matrix(post[,gc, drop=FALSE]),2,quantile,c(.025,.975))

    mk <- function(pt,pn,tv,e,v,lo,hi)
      data.table(condition=m,parameter=pn,param_type=pt,
                 true_val=tv,est_mean=e,est_var=v,ci_lo=lo,ci_hi=hi,
                 bias=e-tv,sq_err=(e-tv)^2,
                 covered=as.numeric(tv>=lo & tv<=hi))

    results[[length(results)+1]] <- mk("baseline","beta0",TRUE_BETA0[m],
      gm[1],gv[1],gci[1,1],gci[2,1])

    infl <- setdiff(CONDS,m)
    tmp  <- as.data.frame(matrix(0,1,length(infl))); colnames(tmp) <- infl
    ir   <- build_interaction_cols(tmp, infl, p_fit)
    n_m  <- length(infl)

    for (idx in seq_along(infl)) {
      j <- infl[idx]; key <- paste(m,j,sep="|")
      tv <- if (key %in% names(TRUE_BETA_MAIN)) TRUE_BETA_MAIN[key] else 0
      if (idx>length(bm)) next
      results[[length(results)+1]] <- mk("main_effect",paste0("b_",j),tv,
        bm[idx],bv[idx],bci[1,idx],bci[2,idx])
    }

    if (p_fit>=2L) {
      two_cols <- ir$cols[ir$orders==1]
      for (ic in seq_along(two_cols)) {
        pts <- strsplit(two_cols[ic],"_x_")[[1]]
        j<-pts[1];k<-pts[2]
        k1<-paste(m,j,k,sep="|");k2<-paste(m,k,j,sep="|")
        tv <- if (k1 %in% names(TRUE_BETA_INT2)) TRUE_BETA_INT2[[k1]]
              else if (k2 %in% names(TRUE_BETA_INT2)) TRUE_BETA_INT2[[k2]] else 0
        pos <- n_m+ic; if (pos>length(bm)) next
        results[[length(results)+1]] <- mk("interaction_2way",
          paste0("b2_",j,"_",k),tv,bm[pos],bv[pos],bci[1,pos],bci[2,pos])
      }
    }

    if (p_fit>=3L) {
      three_cols <- ir$cols[ir$orders==2]
      off3       <- n_m+sum(ir$orders==1)
      for (ic in seq_along(three_cols)) {
        pts <- strsplit(three_cols[ic],"_x_")[[1]]
        j<-pts[1];k<-pts[2];l<-pts[3]
        keys <- apply(permn_3(c(j,k,l)),1,function(x) paste(c(m,x),collapse="|"))
        mk_k <- keys[keys %in% names(TRUE_BETA_INT3)]
        tv   <- if (length(mk_k)>0) TRUE_BETA_INT3[[mk_k[1]]]
                else if (p_true>=3L) 0 else NA_real_
        pos  <- off3+ic; if (pos>length(bm)||is.na(tv)) next
        results[[length(results)+1]] <- mk("interaction_3way",
          paste0("b3_",j,"_",k,"_",l),tv,bm[pos],bv[pos],bci[1,pos],bci[2,pos])
      }
    }

    gnames <- c("intercept","sex_male","smk_current","smk_former","age")
    tg     <- c(0,TRUE_GAMMA[[m]]); names(tg) <- gnames
    for (gi in 2:min(length(gm),length(gnames))) {
      gn <- gnames[gi]; tvg <- tg[gn]
      results[[length(results)+1]] <- mk("covariate",paste0("g_",gn),tvg,
        gm[gi],gv[gi],gci[1,gi],gci[2,gi])
    }
  }
  rbindlist(results)
}

compute_pred_metrics <- function(fit, DT_test, eval_times=EVAL_TIMES) {
  results <- list()
  for (m in CONDS) {
    ec  <- paste0(m,"_event")
    atr <- DT_test[get(m)==0 & dt>0]
    if (nrow(atr)==0 || !ec %in% names(atr)) next
    pred   <- tryCatch(get_lp(fit,atr,m),
                       error=function(e) list(lp=rep(NA,nrow(atr)),
                                              lambda=rep(NA,nrow(atr))))
    y      <- atr[[ec]]; dt_ <- atr$dt
    lam    <- pred$lambda; lp <- pred$lp
    mu     <- lam * pmax(dt_,1e-10)
    p_hat  <- 1 - exp(-pmax(mu,0))
    pll    <- mean(dpois(y,pmax(mu,1e-15),log=TRUE),na.rm=TRUE)
    brier  <- mean((y-p_hat)^2,na.rm=TRUE)
    pd     <- data.table(event=y,t_ev=atr$time_to_event,
                         marker=lp+log(pmax(dt_,1e-10)))[is.finite(marker)]
    av     <- setNames(rep(NA_real_,length(eval_times)),as.character(eval_times))
    if (nrow(pd)>1 && length(unique(pd$event))==2) {
      td <- tryCatch(
        timeROC::timeROC(T=pd$t_ev,delta=pd$event,marker=pd$marker,
                         cause=1,times=eval_times,iid=FALSE),
        error=function(e) NULL)
      if (!is.null(td)) av <- setNames(td$AUC,as.character(eval_times))
    }
    results[[m]] <- data.table(condition=m,pll=pll,brier=brier,as.list(av))
  }
  rbindlist(results,fill=TRUE)
}

# --------------------------------------------------------------------------- #
# 6.  One-replicate worker function
# --------------------------------------------------------------------------- #

run_one_replicate <- function(r, scenario, p_fit, p_true, theta, prior,
                               k_fold, n_sim, eval_times, base_seed) {
  seed_r       <- base_seed + r * 1000L
  include_3way <- p_true >= 3L
  rep_path     <- file.path("sim_results/replicates",
                             sprintf("rep_%s_%03d.rds", scenario, r))
  t_start      <- proc.time()["elapsed"]

  rep_log(scenario, r, sprintf(
    "START | p_fit=%d p_true=%d theta=%.1f seed=%d",
    p_fit, p_true, theta, seed_r))

  # ── Generate data ─────────────────────────────────────────────────────── #
  set.seed(seed_r)
  rep_log(scenario, r, "Generating dataset...")
  DT_raw  <- generate_dataset(n=n_sim, seed=seed_r, include_3way=include_3way)
  DT_wide <- prepare_wide(DT_raw)

  setorder(DT_wide, eid, time_to_event)
  for (cond in CONDS) {
    ec <- paste0(cond,"_event")
    DT_wide[,(ec) := as.numeric(
      get(cond)==0 & shift(get(cond),type="lead")==1), by=eid]
    DT_wide[is.na(get(ec)),(ec) := 0L]
  }

  # ── Full-data fit: recovery + selection ───────────────────────────────── #
  rep_log(scenario, r, "Fitting full-data model...")
  fit_full <- tryCatch(
    ctbn_fit(DT_wide, prior=prior, max_order=p_fit,
             fixed_covs=c("sex_male","smk_current","smk_former"),
             time_varying_covs="age",
             variable_select=FALSE,
             theta=theta, chains=2L, iter=1000L, warmup=500L,
             seed=seed_r, verbose=FALSE),
    error=function(e){ rep_log(scenario,r,conditionMessage(e),"WARN"); NULL })

  recovery_dt  <- if (!is.null(fit_full)) {
    rep_log(scenario, r, "Computing recovery metrics...")
    compute_recovery_metrics(fit_full, p_true=p_true)
  } else NULL

  selection_dt <- if (!is.null(fit_full)) {
    rep_log(scenario, r, "Computing selection metrics...")
    compute_selection_metrics(fit_full, p_true=p_true)
  } else NULL

  # ── K-fold CV: predictive + oracle ───────────────────────────────────── #
  eids     <- unique(DT_wide$eid)
  set.seed(seed_r + 1L)
  fold_ids <- sample(rep(seq_len(k_fold), length.out=length(eids)))
  fold_map <- data.table(eid=eids, fold=fold_ids)
  DT_wide  <- merge(DT_wide, fold_map, by="eid")

  pred_list <- oracle_list <- list()

  for (k in seq_len(k_fold)) {
    rep_log(scenario, r, sprintf("Fold %d/%d — fitting...", k, k_fold))
    DT_tr <- DT_wide[fold!=k][, fold:=NULL]
    DT_te <- DT_wide[fold==k][, fold:=NULL]

    fit_k <- tryCatch(
      ctbn_fit(DT_tr, prior=prior, max_order=p_fit,
               fixed_covs=c("sex_male","smk_current","smk_former"),
               time_varying_covs="age",
               variable_select=FALSE,
               theta=theta, chains=2L, iter=1000L, warmup=500L,
               seed=seed_r+k, verbose=FALSE),
      error=function(e){ rep_log(scenario,r,conditionMessage(e),"WARN"); NULL })

    if (!is.null(fit_k)) {
      rep_log(scenario, r, sprintf("Fold %d/%d — computing metrics...", k, k_fold))
      pm <- compute_pred_metrics(fit_k, DT_te, eval_times)
      pm[, `:=`(fold=k,scenario=scenario,p_fit=p_fit,p_true=p_true,theta=theta)]
      pred_list[[k]] <- pm

      om <- compute_oracle_metrics(DT_te, include_3way=include_3way)
      om[, `:=`(fold=k,scenario=scenario,p_fit=p_fit,p_true=p_true,theta=theta)]
      oracle_list[[k]] <- om
    }
  }
  DT_wide[, fold := NULL]

  wall_time <- proc.time()["elapsed"] - t_start
  rep_log(scenario, r, sprintf("DONE in %s", format_duration(wall_time)))

  result <- list(
    rep          = r,
    scenario     = scenario,
    p_fit        = p_fit,
    p_true       = p_true,
    theta        = theta,
    wall_time    = wall_time,
    recovery_dt  = recovery_dt,
    selection_dt = selection_dt,
    pred_dt      = if (length(pred_list)>0)   rbindlist(pred_list,   fill=TRUE) else NULL,
    oracle_dt    = if (length(oracle_list)>0)  rbindlist(oracle_list, fill=TRUE) else NULL
  )
  saveRDS(result, rep_path)
  result
}

# --------------------------------------------------------------------------- #
# 7.  Main orchestrator with progress monitoring
# --------------------------------------------------------------------------- #

run_simulation <- function(scenario    = "A2",
                            n_rep      = N_REP,
                            n_sim      = N_SIM,
                            k_fold     = K_FOLD,
                            p_fit      = 2L,
                            p_true     = 2L,
                            prior      = PRIOR_FIT,
                            theta      = THETA_FIT,
                            n_cores    = 20L,
                            eval_times = EVAL_TIMES,
                            base_seed  = SIM_SEED,
                            poll_secs  = 5) {

  # ── Identify which replicates still need to run ───────────────────────── #
  rep_paths <- file.path("sim_results/replicates",
    sprintf("rep_%s_%03d.rds", scenario, seq_len(n_rep)))
  todo <- which(!file.exists(rep_paths))
  done_pre <- n_rep - length(todo)

  cat(sprintf(
    "\n%s === Scenario %s | P_fit=%d | P_true=%d | theta=%.1f ===\n",
    format(Sys.time(),"%H:%M:%S"), scenario, p_fit, p_true, theta))
  cat(sprintf("  Replicates: %d total | %d already done | %d to run\n\n",
              n_rep, done_pre, length(todo)))

  if (length(todo) == 0) {
    cat("  Nothing to do — all replicates already completed.\n")
    return(invisible(load_scenario(scenario, n_rep)))
  }

  # ── Initialise progress tracker ───────────────────────────────────────── #
  prog <- SimProgress$new(scenario, n_rep, k_fold, M)

  # ── Set up parallel cluster ───────────────────────────────────────────── #
  n_actual <- min(n_cores, length(todo))
  cl <- makeCluster(n_actual,
                    outfile = file.path("sim_results/logs",
                                        sprintf("cluster_%s.log", scenario)))
  registerDoParallel(cl)

  # Export everything needed by workers
  export_vars <- c(
    "CONDS","M","TRUE_BETA0","TRUE_GRAPH","TRUE_BETA_MAIN",
    "TRUE_BETA_INT2","TRUE_BETA_INT3","TRUE_GAMMA","T_HORIZON",
    "generate_dataset","prepare_wide","simulate_patient",
    "compute_q_true","sim_smoking_step","smk_to_covs",
    "compute_recovery_metrics","compute_selection_metrics",
    "compute_pred_metrics","compute_oracle_metrics",
    "run_one_replicate","build_interaction_cols","get_lp",
    "permn_3","rep_log","format_duration",
    "PIP_THRESH","EVAL_TIMES","N_SIM"
  )

  # ── Launch parallel jobs (non-blocking via future-like polling) ───────── #
  # We use a shared-memory trick: foreach returns results AND each worker
  # saves its own RDS. The main process polls the progress file every
  # poll_secs seconds and prints the dashboard.

  cat(sprintf("  Starting %d parallel workers...\n\n", n_actual))

  # Parallel execution — results also written to individual RDS by worker
  future_results <- foreach(
    r          = todo,
    .packages  = c("data.table","rstan","timeROC","pROC"),
    .export    = export_vars,
    .errorhandling = "pass"
  ) %dopar% {
    tryCatch({
      result <- run_one_replicate(
        r=r, scenario=scenario, p_fit=p_fit, p_true=p_true,
        theta=theta, prior=prior, k_fold=k_fold, n_sim=n_sim,
        eval_times=eval_times, base_seed=base_seed)
      prog$increment(r, result$wall_time)
      result
    }, error = function(e) {
      prog$record_failure(r, conditionMessage(e))
      list(rep=r, error=conditionMessage(e))
    })
  }

  # Note: foreach blocks until all workers finish, so we print the dashboard
  # AFTER the loop. For true non-blocking progress in an interactive session,
  # use the polling wrapper below instead.
  stopCluster(cl)
  registerDoSEQ()

  # ── Final dashboard ───────────────────────────────────────────────────── #
  cat("\n")
  prog$final_report()

  # ── Collect and stack all replicate results ───────────────────────────── #
  all_results <- load_scenario(scenario, n_rep)
  cat(sprintf("\n  Loaded %d results.\n", length(all_results)))

  # ── Write summary log ─────────────────────────────────────────────────── #
  summary_path <- file.path("sim_results/logs",
    sprintf("summary_%s_p%d_t%02d.log", scenario, p_fit, round(theta*10)))
  state <- prog$final_report()
  cat(sprintf("\nSummary written to: %s\n", summary_path))
  write(
    sprintf("Scenario %s | P_fit=%d | P_true=%d | theta=%.1f\nCompleted: %d/%d | Failed: %d",
            scenario, p_fit, p_true, theta,
            state$completed + done_pre, n_rep, state$failed),
    summary_path)

  invisible(all_results)
}


# ── Interactive polling wrapper (optional — for R notebooks / Rscript) ────── #
#
# USAGE:
#   run_simulation_with_live_progress(
#     script_path = "ctbn_simulation_v3.R",   # <-- path to THIS file
#     scenario    = "A1",
#     p_fit       = 1L,
#     p_true      = 2L,
#     theta       = 1.0,
#     n_cores     = 20L
#   )
#
# HOW IT WORKS:
#   callr::r_bg() starts a completely fresh R process with no inherited
#   environment. We therefore must:
#     1. Pass script_path so the subprocess can source() all functions.
#     2. Pass all required global constants as explicit args.
#     3. Inside the subprocess, source() the script first, then call
#        run_simulation() with the provided args.
#   The main process polls the shared progress JSON file every poll_secs
#   seconds and prints the live dashboard. When the background process
#   finishes, the main process prints the final report.

run_simulation_with_live_progress <- function(script_path,
                                               scenario    = "A2",
                                               p_fit       = 2L,
                                               p_true      = 2L,
                                               theta       = 1.0,
                                               prior       = PRIOR_FIT,
                                               n_rep       = N_REP,
                                               n_sim       = N_SIM,
                                               k_fold      = K_FOLD,
                                               n_cores     = 20L,
                                               eval_times  = EVAL_TIMES,
                                               base_seed   = SIM_SEED,
                                               poll_secs   = 5,
                                               ctbn_script = "ctbn_all_codes.R") {

  if (!requireNamespace("callr", quietly = TRUE))
    stop("Install callr first:  install.packages('callr')")

  # Resolve to absolute paths so the subprocess can find them from any wd
  script_path <- normalizePath(script_path, mustWork = TRUE)
  ctbn_script <- normalizePath(ctbn_script, mustWork = TRUE)

  # All sim_results sub-dirs must exist before the background process starts
  for (d in c("sim_results","sim_results/replicates","sim_results/figures",
              "sim_results/tables","sim_results/logs","sim_results/progress"))
    dir.create(d, showWarnings = FALSE, recursive = TRUE)

  # ── Assemble args to pass to background process ───────────────────────── #
  bg_args <- list(
    script_path = script_path,
    ctbn_script = ctbn_script,
    scenario    = scenario,
    p_fit       = p_fit,
    p_true      = p_true,
    theta       = theta,
    prior       = prior,
    n_rep       = n_rep,
    n_sim       = n_sim,
    k_fold      = k_fold,
    n_cores     = n_cores,
    eval_times  = eval_times,
    base_seed   = base_seed
  )

  # ── Background worker function ────────────────────────────────────────── #
  # This is the ONLY code that runs in the fresh subprocess.
  # It sources both scripts (which loads all functions and constants),
  # then calls run_simulation() with the provided arguments.
  bg_func <- function(args) {
    # Source scripts in correct order — this populates all functions,
    # constants (CONDS, TRUE_BETA0, etc.) and the SimProgress class.
    source(args$ctbn_script,  local = FALSE)
    source(args$script_path,  local = FALSE)

    # Call the blocking run_simulation() — progress is written to JSON
    # files which the main process reads via the polling loop.
    run_simulation(
      scenario   = args$scenario,
      n_rep      = args$n_rep,
      n_sim      = args$n_sim,
      k_fold     = args$k_fold,
      p_fit      = args$p_fit,
      p_true     = args$p_true,
      prior      = args$prior,
      theta      = args$theta,
      n_cores    = args$n_cores,
      eval_times = args$eval_times,
      base_seed  = args$base_seed
    )
  }

  # ── Launch background process ─────────────────────────────────────────── #
  bg <- callr::r_bg(
    func      = bg_func,
    args      = list(args = bg_args),
    supervise = TRUE,
    stdout    = file.path("sim_results/logs",
                           sprintf("bg_stdout_%s.log", scenario)),
    stderr    = file.path("sim_results/logs",
                           sprintf("bg_stderr_%s.log", scenario))
  )

  cat(sprintf(
    "\n%s  Scenario %s launched in background (PID %d)\n",
    format(Sys.time(), "%H:%M:%S"), scenario, bg$get_pid()))
  cat(sprintf(
    "  stdout -> sim_results/logs/bg_stdout_%s.log\n", scenario))
  cat(sprintf(
    "  stderr -> sim_results/logs/bg_stderr_%s.log\n\n", scenario))

  # ── Initialise progress tracker in main process ───────────────────────── #
  prog <- SimProgress$new(
    scenario = scenario,
    n_rep    = n_rep,
    k_fold   = k_fold,
    n_cond   = length(CONDS)   # CONDS available in main process
  )

  # Give the subprocess a moment to create the progress file
  Sys.sleep(2)

  # ── Polling loop ──────────────────────────────────────────────────────── #
  while (bg$is_alive()) {
    prog$print_dashboard()
    Sys.sleep(poll_secs)
  }

  # One final dashboard update after process exits
  prog$print_dashboard()
  cat("\n")

  # ── Check exit status ─────────────────────────────────────────────────── #
  exit_ok <- tryCatch({
    bg$get_result()   # re-raises any error from the subprocess
    TRUE
  }, error = function(e) {
    cat(sprintf("\n  [ERROR] Background process failed: %s\n",
                conditionMessage(e)))
    cat(sprintf("  Check: sim_results/logs/bg_stderr_%s.log\n", scenario))
    FALSE
  })

  prog$final_report()

  if (exit_ok)
    cat("\n  Background process completed successfully.\n")

  invisible(load_scenario(scenario, n_rep))
}

`%||%` <- function(a, b) if (!is.null(a)) a else b


# ── Load completed replicates for a scenario ─────────────────────────────── #

load_scenario <- function(scenario, n_rep = N_REP) {
  paths <- file.path("sim_results/replicates",
    sprintf("rep_%s_%03d.rds", scenario, seq_len(n_rep)))
  exists_idx <- which(file.exists(paths))
  cat(sprintf("  Loading %d/%d replicates for scenario %s...\n",
              length(exists_idx), n_rep, scenario))
  lapply(paths[exists_idx], readRDS)
}


# --------------------------------------------------------------------------- #
# 8.  Run all scenarios (uncomment as needed)
# --------------------------------------------------------------------------- #
# Each call is resumable: already-completed replicates are skipped.
#
# ── Option A: blocking (no live dashboard, simpler) ──────────────────────── #
#
# Scenario A1: truth P=2, fit P=1
# run_simulation("A1", p_fit=1L, p_true=2L, theta=1.0, n_cores=20L)
#
# Scenario A2: truth P=2, fit P=2 (BASE CASE)
# run_simulation("A2", p_fit=2L, p_true=2L, theta=1.0, n_cores=20L)
#
# Scenario A3: truth P=3, fit P=3
# run_simulation("A3", p_fit=3L, p_true=3L, theta=1.0, n_cores=20L)
#
# Scenario C:  truth P=3, fit P=2 (MISSPECIFIED)
# run_simulation("C",  p_fit=2L, p_true=3L, theta=1.0, n_cores=20L)
#
# Theta sensitivity:
# for (th in c(0.5, 2.0)) {
#   run_simulation("A2", p_fit=2L, p_true=2L, theta=th, n_cores=20L)
#   run_simulation("A3", p_fit=3L, p_true=3L, theta=th, n_cores=20L)
# }
#
# ── Option B: live dashboard (requires callr) ─────────────────────────────── #
# You MUST supply script_path = path to THIS file so the background process
# can source() it. Both paths are resolved to absolute paths automatically.
#
# run_simulation_with_live_progress(
#   script_path = "ctbn_simulation_v3.R",    # path to this file
#   ctbn_script = "ctbn_results_v4.R",        # path to ctbn_fit() etc.
#   scenario    = "A1",
#   p_fit       = 1L,
#   p_true      = 2L,
#   theta       = 1.0,
#   n_cores     = 20L,
#   poll_secs   = 5       # dashboard refresh interval in seconds
# )
#
# run_simulation_with_live_progress(
#   script_path = "ctbn_simulation_v3.R",
#   ctbn_script = "ctbn_results_v4.R",
#   scenario    = "A2",
#   p_fit       = 2L,
#   p_true      = 2L,
#   theta       = 1.0,
#   n_cores     = 20L
# )
#
# run_simulation_with_live_progress(
#   script_path = "ctbn_simulation_v3.R",
#   ctbn_script = "ctbn_results_v4.R",
#   scenario    = "A3",
#   p_fit       = 3L,
#   p_true      = 3L,
#   theta       = 1.0,
#   n_cores     = 20L
# )
#
# run_simulation_with_live_progress(
#   script_path = "ctbn_simulation_v3.R",
#   ctbn_script = "ctbn_results_v4.R",
#   scenario    = "C",
#   p_fit       = 2L,
#   p_true      = 3L,
#   theta       = 1.0,
#   n_cores     = 20L
# )
